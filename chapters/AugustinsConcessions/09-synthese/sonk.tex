% After the Erkenntnis submission, I went through this version carefully
% and made further cuts (see sonk.org for the cuts). As of <2016-12-04
% Sun>, this is again the one and only official version. I will now
% submit this to Philosophers' Imprint.

% This is the official version. Use it for the dissertation. For
% publication, however, consider the cuts made in sonk-bjps.tex.

% I guess you can consider the cuts made in sonk-bjps, but now I've made
% a lot of changes to this file that are not reflected there.
...
% More formally speaking,\ecut{5} let $\Omega$ be a set of state
...
\qeins{psychologically implausible and epistemologically calamitous,}
see \scite{8}{joyce05}{156}). From the perspective of information
...
{\anderson} and {\augustin} (for example \scite{7}{kaplan10}{};
\scite{7}{hajeksmithson12}{}; \scite{7}{moss13}{};
\scite{7}{chandler14}{}; and \scite{7}{weisberg15}{}).
...
The\ecut{1} superiority of the Boolean approach over the Laplacean
...
% There is a section expanding on Klir/Mork in klirmork.tex, which has
% been cut from here.
...
\scite{8}{mork13}{363}), weak and strong monotonicity. Klir's
...
whose bias we know that it is either 1/3 or 2/3 for heads.\ecut{2}
...
\scite{8}{bradleysteele16}{6}). Logan's doxastic state, however, is
...
provide \qeins{a complete characterization of one's doxastic attitude}
\scite{2}{rinard15}{5} and reiterates a few pages later that it is
\qeins{a primary purpose of the set of functions model to represent
  the totality of the agent's actual doxastic state}
\scite{2}{rinard15}{12}.
...
the time of the day).\bcut{22} The integration proceeds by the
following summation formula (see \scite{8}{lewis81}{266f}),
...
that I will introduce in section \ref{AugustinsConcessions}.\ecut{6}

While most examples in the literature (as one representative, see
\scite{8}{kaplan10}{43f}) present {\anderson} as an amalgamated
...
\scite{8}{white10}{183}).\ecut{7} The problems that dilation causes
...
\item \textsc{retention} This is the problem in White's chocolates
  example (see \scite{8}{white10}{183}). When we tie the outcome of a
...
in \scite{8}{joyce10}{294}, but $\mathcal{C}'$ and $\mathcal{C}''$ do
not generate the same range of probabilities: if, as Joyce says,
$c(Z)\geq1/2$, then $c(X)=c(Y)$ implies $c(X)\leq{}1/4$ for
$\mathcal{C}''$, but not for $\mathcal{C}'$).\ecut{8} What Joyce wants
...
in the abstract of his paper that reactions to dilation \qeins{are
  ...
  values for her imprecise credences.} Not only does Joyce's example
...
argument \qeins{goes awry by assuming that credal states which assign
  ...
  about that event} \scite{2}{joyce10}{304}, when in the following he
...
(besides Joyce, see also \scite{8}{topey12}{483}).\ecut{3} Credal
...
\scite{7}{shorejohnson80}{}; \scite{7}{giffin08}{}; and
\scite{7}{lukits15}{}). Furthermore, the Laplacean position is
...
\scite{8}{white10}{183}; or how a ball is retrieved from an urn as in
