...
$tmax=20;
$smax=20;
open(output,">./conjecture-280-1.htm");
...
{
    $t=$tmin+($ts1*$tstep);
    print output "<td><font color=\"red\"><tt>t=$t</font></tt></td>";
}
print output "</tr>";
for ($ss1=0;$ss1<=$ssmax;++$ss1)
{
    $s=$smin+($ss1*$sstep);
    print output "<tr><td><font color=\"blue\"><tt>s=$s</font></tt></td>";
for ($ts1=0;$ts1<=$tsmax;++$ts1)
{
    $t=$tmin+($ts1*$tstep);
...
print "$t $s $i\n";
    $u1=(($t*($i+.5))-($n/2))/(sqrt($n/2));
    $u2=(($t*($i-.5))-($n/2))/(sqrt($n/2));
    $v1=(($t*$i)+.5-($n/2))/(sqrt($n/2));
    $v2=(($t*$i)-.5-($n/2))/(sqrt($n/2));
    $u=erf($u1)-erf($u2);
    $v=$t*(erf($v1)-erf($v2));
