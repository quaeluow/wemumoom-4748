...
sub Create4
{
    $n=$t*$s;
    $psi=($t**2)+1;
    $arg=(($t**3)-(1.5*($t**2))+(2*$t)-(2.5))/($psi**2);
    $ex12=.5*(1+$t)*exp($arg)*$s*(1/(sqrt(1-(1/($s*$psi*$t)))));
    $y=$n/($n+$ex12);
    $Fy=sprintf("%.3f",$y);
    print output1 "<td align=\"right\"><tt>$Fy</tt></td>"; 
}
...
sub Table4
{
print output1 "<h4>Table 4 (Power Set Approach)</h4><p>";
print output1 "<table border=5><tr><td><tt></tt></td>";
for ($ts1=0;$ts1<=$tsmax;++$ts1)
{
    $t=$tmin+($ts1*$tstep);
...
open(output1,">./conjecture-375.htm");
...
# file:///Volumes/Kodak/stefan-2011/literatur/essays/InformationEpistemology/conjecture-375.htm
